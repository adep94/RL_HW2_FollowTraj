                                      ...
                                      double _Kp, double _Kd)
{
    ...
    return robot_->getJsim() * (ddqd + _Kd*de + _Kp*e)
            + robot_->getCoriolis() + 0.5*robot_->getGravity() /*friction compensation?*/;
} 
...
//  Eigen::Matrix<double,6,6> Kp, Kd;
    ...
    Eigen::Matrix<double,6,7> J = toEigen(J_ee);
    ...
    Eigen::Matrix<double,6,1> J_dot_q_dot = toEigen(robot_->getEEJacDotqDot());

    // position
    Eigen::Vector3d p_d(_desPos.p.data);    //prova senza .data
    ...
    KDL::Vector effective_dotp = robot_->getEEVelocity().vel; 
    Eigen::Vector3d dot_p_e = toEigen(effective_dotp);
    Eigen::Vector3d omega_d(_desVel.rot.data);
    KDL::Vector effective_omega = robot_->getEEVelocity().rot;
    Eigen::Vector3d omega_e = toEigen(effective_omega);
...
//    std::cout << "---------------------" << std::endl;
    ...
    return M * (Jpinv*y + (I-Jpinv*J)*(/*- 10*grad */- 1*robot_->getJntVelocities()))
            + 0.5*robot_->getGravity() + robot_->getCoriolis();
//    return M * (Jpinv*y) + 0.5*robot_->getGravity() + robot_->getCoriolis();
}
