...
int main(int argc, char **argv)
{
    ...
    // Plan trajectory
    double traj_duration = 10.0, acc_duration = 1.5, t = 0.0, init_time_slot = 1.0, traj_radius = 0.1;
    ...
    while ((ros::Time::now()-begin).toSec() < 2*traj_duration + init_time_slot)
    {
        if (robot_state_available)
        {
            ...
            error_msg.x = error_vector[0];  
            error_msg.y = error_vector[1];
            error_msg.z = error_vector[2];
            ...
            // Gains
            //double Kp = 28, Kd = 8/*sqrt(Kp)*/; //70 7 per la cubica circolare // 60 5 per la trap circ
            ...
            tau = controller_.idCntr(des_pose, des_cart_vel, des_cart_acc,
                                    Kp, Ko, 2*0.8*sqrt(Kp), 2*0.8*sqrt(Ko));
